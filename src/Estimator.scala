...
abstract class Estimator[VD,ED](wholeGraph: Graph[VD,ED]){
  ...
  def ConditionalClose(subEdges:List[Edge[ED]], expectEdges:List[Edge[ED]]) : Boolean={
    ...
    edges.drop(streamIndex+1).foreach(edge =>{
      val edgeTofind = new Edge[ED](edge.srcId, edge.dstId)
       if(findExpect.contains(edgeTofind)){
         findExpect(edgeTofind) = true
       }
    })
...
abstract class sEstimator[VD, ED](wholeGraph: Graph[VD, ED]) {
  var estimationValue = 0.0
  val edges = wholeGraph.edges.collect()
  var streamIndex = 0
  var sampleEdgeList = List[Edge[ED]]()
...
class sTriangleEstimator[VD,ED](wholeGraph: Graph[VD,ED]) extends sEstimator[VD,ED](wholeGraph){
  //specified to count triangles, to be generalized
  override def ConditionalClose(subEdges: List[Edge[ED]], expectEdges: List[Edge[ED]]): Boolean = {
    var vertex: Map[graphx.VertexId,Int] = Map[graphx.VertexId,Int](1.toLong->0,2.toLong->0,3.toLong->0,4.toLong->0,5.toLong->0,6.toLong->0,7.toLong->0)
    for(e <- subEdges){
      vertex += (e.srcId.toLong -> (vertex(e.srcId) + 1).toInt)
      vertex += (e.dstId.toLong -> (vertex(e.dstId) + 1).toInt)
    }
